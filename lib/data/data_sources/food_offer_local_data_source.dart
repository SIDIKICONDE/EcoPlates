...
class FoodOfferLocalDataSourceImpl implements FoodOfferLocalDataSource {
  ...
  late Box<FoodOfferModel> _offersBox;
  late Box<List<String>> _recommendationsBox;
  late Box<Map<String, dynamic>> _pendingActionsBox;
  ...
  Future<void> _ensureBoxesInitialized() async {
    if (!_boxesInitialized) {
      ...
      _recommendationsBox = await Hive.openBox<List<String>>(
        _recommendationsBoxName,
      );
      _pendingActionsBox = await Hive.openBox<Map<String, dynamic>>(
        _pendingActionsBoxName,
      );
  ...
  Future<List<FoodOfferModel>> searchOffers({
    ...
    return allOffers.where((offer) {
      ...
      final matchesQuery =
          offer.title.toLowerCase().contains(queryLower) ||
      ...
      if (filters != null) {
        ...
        if (filters['maxPrice'] != null &&
            offer.discountedPrice > (filters['maxPrice'] as num)) {
          return false;
        }
  ...
  Future<List<Map<String, dynamic>>> getPendingReservations() async {
    ...
    return _pendingActionsBox.values
        .where(
          (action) =>
              action['type'] == 'reservation' && action['synced'] == false,
        )
        .toList();
  }
  ...
  Future<List<String>> getPendingCancellations() async {
    ...
    return _pendingActionsBox.values
        .where(
          (action) =>
              action['type'] == 'cancellation' && action['synced'] == false,
        )
  ...
  Future<void> markCancellationSynced(String cancellationId) async {
    await _ensureBoxesInitialized();
    final cancellation = _pendingActionsBox.values.firstWhere(
      (action) =>
