...
abstract class AssociationRepository {
  ...
  Future<Either<Failure, Association>> updateAssociation(Association association);
  ...
  Future<Either<Failure, AssociationStats>> getAssociationStats(String associationId);
...
class AssociationRepositoryImpl implements AssociationRepository {
  ...
  Future<Either<Failure, Association>> createAssociation({
    ...
    try {
      ...
      final newAssociation = AssociationModel.fromJson(response.data).toEntity();
      return Right(newAssociation);
    } on DioException catch (e) {
      if (e.response?.statusCode == 409) {
        return Left(ConflictFailure(
          'Une association avec ce SIRET existe déjà',
        ));
      }
      return Left(ServerFailure(
  ...
  @override
  Future<Either<Failure, Association>> updateAssociation(Association association) async {
    try {
      ...
      final updatedAssociation = AssociationModel.fromJson(response.data).toEntity();
  ...
  @override
  Future<Either<Failure, AssociationStats>> getAssociationStats(String associationId) async {
    try {
      final response = await dio.get('$baseUrl/associations/$associationId/stats');
  ...
  Future<Either<Failure, GroupCollection>> scheduleGroupCollection({
    ...
    try {
      ...
      final collection = GroupCollectionModel.fromJson(response.data).toEntity();
      return Right(collection);
    } on DioException catch (e) {
      if (e.response?.statusCode == 400) {
        return Left(ValidationFailure(
          'Données de collecte invalides',
        ));
      }
      return Left(ServerFailure(
  ...
  Future<Either<Failure, void>> addVolunteer({
    ...
    try {
      ...
      return Left(ServerFailure(e.message ?? 'Erreur lors de l\'ajout du bénévole',));
    } catch (e) {
  ...
  Future<Either<Failure, void>> removeVolunteer({
    ...
    try {
      ...
      return Left(ServerFailure(e.message ?? 'Erreur lors du retrait du bénévole',));
    } catch (e) {
      return Left(ServerFailure('Une erreur inattendue est survenue',));
    }
  }
