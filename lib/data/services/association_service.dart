...
class AssociationService {
  ...
  Future<Either<Failure, List<FoodOffer>>> getAssociationPriorityOffers({
    ...
    try {
      ...
      final associationResult = await associationRepository.getAssociationById(associationId);
      
      return associationResult.fold(
        (failure) => Left(failure),
        (association) async {
          // Récupérer toutes les offres dans la zone
          final offersResult = await foodOfferRepository.getNearbyOffers(
            latitude: 48.8566, // TODO: Utiliser la vraie localisation de l'association
            longitude: 2.3522,
          ...
          return offersResult.fold(
            ...
            (offers) {
              ...
              final priorityOffers = offers.where((offer) {
      ...
      return await associationResult.fold(
        ...
        (association) async {
          if (!association.canCollectOffers) {
            return Left(ValidationFailure('L\'association n\'est pas autorisée à collecter des offres',));
          }
          ...
          if (validOfferIds.isEmpty) {
            return Left(ValidationFailure('Aucune offre valide pour la collecte',));
          }
  ...
  Future<Either<Failure, AssociationImpactReport>> calculateAssociationImpact({
    ...
    try {
      ...
      final statsResult = await associationRepository.getAssociationStats(associationId);
      
      return await statsResult.fold(
        (failure) => Future.value(Left<Failure, AssociationImpactReport>(failure)),
        (stats) async {
          // Récupérer l'historique des collectes
          final collectionsResult = await associationRepository.getAssociationCollections(
            associationId: associationId,
          ...
          return collectionsResult.fold(
            ...
            (collections) {
              ...
              final report = AssociationImpactReport(
                associationId: associationId,
                period: ImpactPeriod(
                  from: fromDate ?? DateTime.now().subtract(const Duration(days: 365)),
  ...
  Future<Either<Failure, AssociationValidationResult>> validateAssociationDocuments({
    required String associationId,
    ...
    try {
      ...
      final missingChecks = requiredDocs.where(
        (doc) => !documentChecks.containsKey(doc) || documentChecks[doc] == false,
      ).toList();
      
      if (missingChecks.isNotEmpty) {
        return Right(AssociationValidationResult(
          approved: false,
          rejectionReasons: missingChecks.map((doc) => 'Document manquant: $doc').toList(),
          validatedBy: adminId,
      ...
      return result.fold(
        (failure) => Left(failure),
        (association) => Right(AssociationValidationResult(
          approved: true,
          rejectionReasons: [],
          validatedBy: adminId,
          validatedAt: DateTime.now(),
  ...
  Map<String, int> _calculateMonthlyTrend(List<GroupCollection> collections) {
    ...
    for (final collection in collections) {
      final monthKey = '${collection.scheduledAt.year}-${collection.scheduledAt.month.toString().padLeft(2, '0')}';
