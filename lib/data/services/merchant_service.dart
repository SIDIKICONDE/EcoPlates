...
class MerchantService {
  ...
  Future<Either<Failure, void>> updateBusinessStatus({
    ...
    try {
      ...
      final result = await _merchantRepository.updateMerchant(merchantId, updates);
      return result.fold(
      ...
      return offerResult.fold(
        ...
        (offer) async {
          ...
          final duplicatedOffer = FoodOffer(
            ...
            pickupStartTime: DateTime.now().add(const Duration(hours: 2)), // Commencer dans 2h
            pickupEndTime: DateTime.now().add(const Duration(hours: 4)), // Terminer dans 4h
      ...
      return offersResult.fold(
        ...
        (offers) async {
          ...
          return dashboardResult.fold(
            (failure) => Left(failure),
            (dashboard) => Right(MerchantQuickStats(
              ...
              expiringToday: offers.where((o) =>
                o.pickupEndTime.difference(DateTime.now()).inHours <= 24
              ).length,
            )),
          );
        },
