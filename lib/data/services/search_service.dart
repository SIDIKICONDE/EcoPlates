...
class SearchService {
  ...
  List<FoodOffer> searchOffers(List<FoodOffer> offers, String query) {
    ...
    return offers.where((offer) {
      ...
      // Prix
      if (lowerQuery.contains('pas cher') && offer.discountedPrice < 5) score += 50;
      if (lowerQuery.contains('économie') && offer.discountPercentage > 50) score += 50;
  ...
  List<SearchSuggestion> getSuggestions(String query, List<FoodOffer> offers) {
    ...
    if (query.isEmpty) {
      ...
      if ('végétarien'.contains(lowerQuery) || 'vegetarien'.contains(lowerQuery)) {
        suggestions.add(SearchSuggestion(
          text: 'Végétarien',
          type: SuggestionType.filter,
          icon: SearchSuggestionIcon.filter,
