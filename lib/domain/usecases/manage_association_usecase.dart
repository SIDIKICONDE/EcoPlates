...
final getAssociationByIdUseCaseProvider = Provider<GetAssociationByIdUseCase>((ref) {
  return GetAssociationByIdUseCase(ref.watch(associationRepositoryProvider));
});

final getAssociationPriorityOffersUseCaseProvider = Provider<GetAssociationPriorityOffersUseCase>((ref) {
  return GetAssociationPriorityOffersUseCase(ref.watch(associationServiceProvider));
});

final scheduleGroupCollectionUseCaseProvider = Provider<ScheduleGroupCollectionUseCase>((ref) {
  return ScheduleGroupCollectionUseCase(ref.watch(associationServiceProvider));
});

final calculateAssociationImpactUseCaseProvider = Provider<CalculateAssociationImpactUseCase>((ref) {
  return CalculateAssociationImpactUseCase(ref.watch(associationServiceProvider));
});

final validateAssociationUseCaseProvider = Provider<ValidateAssociationUseCase>((ref) {
...
class ScheduleGroupCollectionUseCase {
  ...
  Future<Either<Failure, GroupCollection>> call({
    ...
    if (offerIds.isEmpty) {
      return Left(ValidationFailure('Au moins une offre doit être sélectionnée'));
    }
    
    if (scheduledAt.isBefore(DateTime.now())) {
      return Left(ValidationFailure('La date de collecte doit être dans le futur'));
    }
...
class CalculateAssociationImpactUseCase {
  ...
  Future<Either<Failure, AssociationImpactReport>> call({
    ...
    if (fromDate != null && toDate != null && fromDate.isAfter(toDate)) {
      return Left(ValidationFailure('La date de début doit être avant la date de fin',));
    }
...
class ValidateAssociationUseCase {
  ...
  Future<Either<Failure, Association>> call({
    ...
    if (!approved && (rejectionReason == null || rejectionReason.isEmpty)) {
      return Left(ValidationFailure('Une raison de rejet est requise en cas de refus',));
    }
...
class CreateAssociationUseCase {
  ...
  Future<Either<Failure, Association>> call({
    ...
    if (association.siret.isEmpty || association.siret.length != 14) {
      return Left(ValidationFailure('Le numéro SIRET doit contenir 14 chiffres'));
    }
