...
class ManageOffersUseCase {
    ...
    try {
      ...
      return offerResult.fold((failure) => Left(failure), (offer) async {
        if (offer.merchantId != merchantId) {
          return Left(
            PermissionFailure('Vous ne pouvez pas modifier cette offre'),
          );
        }
        ...
        if (offer.status == OfferStatus.collected) {
          return Left(
            BusinessFailure('Une offre collectée ne peut pas être modifiée'),
          );
        }
  ...
  Future<Either<Failure, void>> deleteOffer({
    ...
    try {
      ...
      return offerResult.fold((failure) => Left(failure), (offer) async {
        if (offer.merchantId != merchantId) {
          return Left(
            PermissionFailure('Vous ne pouvez pas supprimer cette offre'),
          );
        }

        // Vérifier s'il y a des réservations actives
        final hasActiveReservations = await offerRepository
            .hasActiveReservations(offerId);
  ...
  Future<Either<Failure, FoodOffer>> duplicateOffer({
    ...
    try {
      ...
      return offerResult.fold((failure) => Left(failure), (offer) async {
        if (offer.merchantId != merchantId) {
          return Left(
            PermissionFailure('Vous ne pouvez pas dupliquer cette offre'),
          );
        }
        ...
        final request = CreateOfferRequest(
          ...
          pickupStartTime:
              newPickupStartTime ??
              offer.pickupStartTime.add(const Duration(days: 1)),
          pickupEndTime:
              newPickupEndTime ??
              offer.pickupEndTime.add(const Duration(days: 1)),
          images: offer.images,
    ...
    try {
      ...
      return offerResult.fold((failure) => Left(failure), (offer) async {
        ...
        if (activate && offer.status != OfferStatus.draft) {
          return Left(
            BusinessFailure('Seuls les brouillons peuvent être activés'),
          );
        }
        ...
        if (!activate && offer.status != OfferStatus.available) {
          return Left(
            BusinessFailure(
              'Seules les offres disponibles peuvent être désactivées',
            ),
          );
        }
    ...
    try {
      ...
      return offerResult.fold((failure) => Left(failure), (offer) async {
        ...
        if (offer.status != OfferStatus.available) {
          return Left(
            BusinessFailure(
              'Seules les offres disponibles peuvent être modifiées',
            ),
          );
        }

        // Vérifier les réservations existantes
        final activeReservations = await offerRepository
            .getActiveReservationsCount(offerId);
  ...
  String? _validateUpdateData(
    UpdateOfferRequest request,
    FoodOffer currentOffer,
  ) {
    ...
    final discountedPrice =
        request.discountedPrice ?? currentOffer.discountedPrice;
...
class OfferStats {
  ...
  double get completionRate =>
      reservations > 0 ? (completed / reservations) * 100 : 0;
}
