...
class MerchantAnalyticsUseCase {
  ...
  Future<Either<Failure, Analytics>> getAnalytics({
    ...
    try {
      ...
      return merchantResult.fold((failure) => Left(failure), (merchant) async {
        ...
        if (daysDiff > 365) {
          return Left(
            ValidationFailure('La période ne peut pas dépasser 1 an'),
          );
        }
        ...
        return await analyticsRepository.getAnalytics(
          ...
          period: DateRange(start: startDate, end: endDate, type: period),
        );
      });
    } catch (e) {
  ...
  Future<Either<Failure, AnalyticsReport>> generateReport({
    ...
    try {
      ...
      return merchantResult.fold((failure) => Left(failure), (merchant) async {
        ...
        return analyticsResult.fold((failure) => Left(failure), (
          analytics,
        ) async {
  ...
  Future<Either<Failure, PerformanceComparison>> comparePerformance({
    ...
    try {
      ...
      return currentResult.fold((failure) => Left(failure), (current) {
        return previousResult.fold((failure) => Left(failure), (previous) {
          return Right(
  ...
  Future<Either<Failure, Predictions>> getPredictions({
    ...
    try {
      if (daysAhead < 1 || daysAhead > 30) {
        return Left(
          ValidationFailure('Les prédictions sont limitées à 30 jours'),
        );
      }
  ...
  Map<String, PerformanceChange> _calculateChanges(
    Analytics current,
    Analytics previous,
  ) {
    return {
      ...
      'completion_rate': PerformanceChange(
        ...
        changePercent:
            ...
            previous.performance.completionRate,
      ),
...
/// Types de rapports
enum ReportType { performance, financial, ecological, customer, complete }
...
enum ExportDataType { offers, reservations, customers, revenue, ecological }
...
/// Types d'insights
enum InsightType { pricing, timing, inventory, customer, ecological }
